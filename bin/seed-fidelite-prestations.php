...
if ( empty( $posts ) ) {
	...
	tfp_seed_set_field( 'organisation_1_carte', '', $id );
	...
	tfp_seed_set_field( 'organisation_2_carte', '', $id );
	...
	tfp_seed_set_field( 'organisation_3_carte', '', $id );
	...
	tfp_seed_set_field( 'organisation_4_carte', '', $id );
	...
	tfp_seed_set_field( 'organisation_5_carte', '', $id );
	...
	tfp_seed_set_field( 'organisation_6_carte', '1', $id );
	tfp_seed_set_field( 'note_taches', 'Hors prestation courante : maintenance informatique, déplacement de mobilier lourd, nettoyage de moquette en profondeur (shampouinage), qui font l\'objet d\'un devis distinct.', $id );
...
if ( empty( $posts ) ) {
	...
	tfp_seed_set_field( 'organisation_6_carte', '1', $id );
	tfp_seed_set_field( 'note_taches', 'Hors prestation courante : nettoyage de vitrines en hauteur nécessitant une nacelle, entretien de stocks en réserve non accessibles, désinfection réglementée (agroalimentaire).', $id );
	tfp_seed_set_field( 'semaine_titre', 'Une semaine type', $id );
...
if ( empty( $posts ) ) {
	...
	tfp_seed_set_field( 'organisation_6_carte', '1', $id );
	tfp_seed_set_field( 'note_taches', 'Cette prestation est un entretien courant de locaux professionnels. Elle ne comprend ni bio-nettoyage hospitalier, ni stérilisation, ni gestion des DASRI, ni entretien du matériel médical, ni protocole médical spécialisé.', $id );
	tfp_seed_set_field( 'semaine_titre', 'Une semaine type', $id );
...
if ( empty( $posts ) ) {
	...
	tfp_seed_set_field( 'organisation_6_carte', '1', $id );
	tfp_seed_set_field( 'note_taches', 'Le nettoyage du local poubelles et la sortie des conteneurs sont possibles lorsqu\'ils sont prévus et chiffrés dans le devis. Hors prestation : désinsectisation, dératisation, entretien des espaces verts, déneigement, vitrages en hauteur nécessitant un équipement spécialisé.', $id );
	tfp_seed_set_field( 'semaine_titre', 'Une semaine type', $id );
...
if ( empty( $posts ) ) {
	...
	tfp_seed_set_field( 'organisation_6_carte', '1', $id );
	tfp_seed_set_field( 'note_taches', 'Hors prestation : lavage du linge (nous changeons le linge propre fourni par le client, nous ne le lavons pas), petites réparations, accueil et remise de clés aux voyageurs, gestion des réservations, réapprovisionnement acheté par nos soins.', $id );
	tfp_seed_set_field( 'semaine_titre', 'Une semaine type', $id );
...
if ( empty( $posts ) ) {
	...
	tfp_seed_set_field( 'organisation_6_carte', '1', $id );
	tfp_seed_set_field( 'note_taches', 'Hors prestation : évacuation de gravats et déchets de chantier, dépose de matériaux, traitement de surfaces dangereuses (amiante, plomb, moisissures étendues), manipulation de produits classés dangereux, intervention en hauteur avec équipement spécialisé. Ces situations relèvent d\'entreprises spécialisées.', $id );
	tfp_seed_set_field( 'semaine_titre', 'Une semaine type', $id );
